...
class Start extends React.Component{
  ...
  <div>
    ...
    <Slider className="slider-wrapper" autoplay={1000}>
      {content.map((item, index) => (
        <div
          ...
          style={{ background: `url('bk2.jpg') no-repeat center center` }}
        >
          <div className="inner">
            <h1>{item.title}</h1>
            <p>{item.description}</p>
            <button className="but">{item.button}</button>
          </div>
          <section>
    ...
    <button className="but" onClick={()=> this.props.history.push("/Products/Featured")}>hello</button>       
